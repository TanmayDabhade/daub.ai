            ...
            {AGENT_PERF.map((a, i) => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                  ...
                  <span style={{ fontWeight: 500, textTransform: "capitalize", color: "var(--fg)" }}>
                    {a.ag.toLowerCase()}
                  </span>
                  <span style={{ flex: 1 }} />
                  <span className="chip">{a.model}</span>
                </div>
                <div style={{ display: "flex", gap: 12, marginTop: 6, fontSize: 11 }} className="mute">
                  <span>n={a.n}</span>
        ...
        <div className="card">
          ...
          <div style={{ padding: "10px 14px", fontSize: 11 }}>
            {[
              ...
              <div key={r.t} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
                <span className="tkr" style={{ width: 44 }}>{r.t}</span>
                <span className="mute mono" style={{ width: 46, fontSize: 10 }}>{r.age}</span>
                ...
                <span className="mono" style={{ width: 32, fontSize: 10.5, textAlign: "right" }}>
                  {r.c.toFixed(2)}
                </span>
              </div>
            ))}
          ...
          {stream.map((e, i) => (
            <div
              ...
              style={{
                ...
                display: "grid",
                gridTemplateColumns: "64px 80px 54px 16px 1fr 70px",
                gap: 10,
                alignItems: "start",
                fontSize: 12,
              }}
            >
              <span className="mute mono" style={{ fontSize: 10.5 }}>{e.ts}</span>
              <span className="pill acc" style={{ fontSize: 9.5, alignSelf: "start" }}>{e.ag.toLowerCase()}</span>
              <span className="tkr">{e.tk}</span>
              <span className={e.sig === "+" ? "up" : e.sig === "-" ? "down" : "acc"} style={{ fontWeight: 700 }}>
                {e.sig}
              </span>
              <span style={{ lineHeight: 1.55, color: "var(--fg)" }}>{e.msg}</span>
              <span><ConfBar v={e.conf} w={42} /></span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div style={{ padding: "12px 14px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <div style={{ border: "1px solid var(--line-2)", padding: 10, borderRadius: "var(--r-sm)" }}>
                <div className="mute" style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>Sentiment</div>
                <div className="mono down" style={{ fontSize: 20, fontWeight: 600, marginTop: 2 }}>−0.60</div>
                <div className="dim" style={{ fontSize: 10.5, marginTop: 4, lineHeight: 1.45 }}>
                  FTC probe · Reality Labs burn · ad pricing softness
                </div>
              </div>
              <div style={{ border: "1px solid var(--line-2)", padding: 10, borderRadius: "var(--r-sm)" }}>
                <div className="mute" style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>Earnings</div>
                <div className="mono up" style={{ fontSize: 20, fontWeight: 600, marginTop: 2 }}>+0.20</div>
                <div className="dim" style={{ fontSize: 10.5, marginTop: 4, lineHeight: 1.45 }}>
            ...
            {TOKEN_USAGE.map((r, i) => (
              <div key={i}>
                <div style={{ display: "flex" }}>
                  ...
                  <span className="mono" style={{ marginLeft: 10, width: 44, textAlign: "right" }}>{r.cost}</span>
                </div>
